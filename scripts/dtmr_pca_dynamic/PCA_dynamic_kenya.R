...
load("/Users/marcvalenti/TFM_data/dtmr_kenya.RData")
...
rename <- function(x,y){
  if (x < 10) {
    return(name <- paste(getwd(),"_data/img_gif/kenya/",y,"-",'0',x,'-plot.png',sep=''))
  }
  return(name <- paste(getwd(),"_data/img_gif/kenya/",y,"-",x,'-plot.png', sep=''))
...
ptners_high <- names(sort(table(dt$partner_id),decreasing = TRUE)[1:9])
...
#   prcomp()
# save(p_comp_kenya, file = "/Users/marcvalenti/TFM_data/princomp_res_kenya.RData")
...
for (i in 1:nrow(month_year_aux)){
  ...
  dt_plot <- p_comp_kenya$x[idx,1:4] %>%
    as.data.frame() %>%
  ...
  #                              xlim=c(min(p_comp_kenya$x[,1]),max(p_comp_kenya$x[,1])), 
  #                              ylim=c(min(p_comp_kenya$x[,2]),max(p_comp_kenya$x[,2])), 
  #                              zlim=c(min(p_comp_kenya$x[,3]),max(p_comp_kenya$x[,3])),
  ...
  p1 <- ggplot(dt_plot,
    ...
    scale_x_continuous(limits = c(min(p_comp_kenya$x[,1]),
                                  max(p_comp_kenya$x[,1]))) + 
    scale_y_continuous(limits = c(min(p_comp_kenya$x[,2]),
                                  max(p_comp_kenya$x[,2]))) + 
  ...
  p2 <- ggplot(dt_plot,
               aes(x = D3,
                   y = D4,
                   color = dt_plot$which_partner)) + 
    scale_x_continuous(limits = c(min(p_comp_kenya$x[,3]),
                                  max(p_comp_kenya$x[,3]))) + 
    scale_y_continuous(limits = c(min(p_comp_kenya$x[,4]),
                                  max(p_comp_kenya$x[,4]))) + 
    geom_point() + 
    scale_color_manual(values = cols) + 
    labs(color = "Partner") + 
    theme_economist_white()
