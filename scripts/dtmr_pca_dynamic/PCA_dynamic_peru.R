...
load("/Users/marcvalenti/TFM_data/dtmr_peru.RData")
...
rename <- function(x){
  if (x < 10) {
    return(name <- paste(getwd(),"_data/img_gif/peru/plot_",'0',x,'.png',sep=''))
  }
  return(name <- paste(getwd(),"_data/img_gif/peru/plot_",x,'.png', sep=''))
...
#   prcomp()
# save(p_comp_peru, file = "/Users/marcvalenti/TFM_data/princomp_res_peru.RData")
...
for (i in 1:nrow(month_year_aux)){
  ...
  #                              xlim=c(min(p_comp_peru$x[,1]),max(p_comp_peru$x[,1])), 
  #                              ylim=c(min(p_comp_peru$x[,2]),max(p_comp_peru$x[,2])), 
  #                              zlim=c(min(p_comp_peru$x[,3]),max(p_comp_peru$x[,3])),
  ...
  p1 <- ggplot(dt_plot,
    ...
    scale_x_continuous(limits = c(min(p_comp_peru$x[,1]),
                                  max(p_comp_peru$x[,1]))) + 
    scale_y_continuous(limits = c(min(p_comp_peru$x[,2]),
                                  max(p_comp_peru$x[,2]))) + 
  ...
  p2 <- ggplot(dt_plot,
               aes(x = D3,
                   y = D4,
                   color = dt_plot$which_partner)) + 
    scale_x_continuous(limits = c(min(p_comp_peru$x[,3]),
                                  max(p_comp_peru$x[,3]))) + 
    scale_y_continuous(limits = c(min(p_comp_peru$x[,4]),
                                  max(p_comp_peru$x[,4]))) + 
    geom_point() + 
    scale_color_manual(values = cols) + 
    labs(color = "Partner") + 
  ...
  grid.arrange(
    ...
    layout_matrix = rbind(c(1,2),
                          c(1,3)),
    top = text_grob(paste0("Country: Peru - ", "Month ", month_year_aux[i,'month'],"; ", "Year ", month_year_aux[i,'year']," (N = ",nrow(dt_plot),")"),
                    face = "bold",size = 16)
    # bottom = textGrob(
