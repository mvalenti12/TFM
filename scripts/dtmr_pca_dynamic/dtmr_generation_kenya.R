...
dt_kenya_partners<- dt_kenya %>%
  group_by(partner_id,year) %>%
  summarise(n_partner=n()) %>%
  filter(n_partner>500)

#required to obtain the names
# partners_aux<-aggregate(n_partner~partner_id,dt_kenya_partners,FUN=sum) %>%
#   top_n(10,n_partner) %>%
#   mutate(name=NA)
# 
# description<-list()
...
# }
# 
# save(partners_aux,file="/Users/marcvalenti/TFM_data/Names_partners_kenya.Rda")

dt_kenya_partners<-dt_kenya_partners %>%
  left_join(partners_aux[,c(1,3)],by="partner_id")

ggplot(dt_kenya_partners[!is.na(dt_kenya_partners$name),],
       aes(x = year,
           y = n_partner,
           fill = as.factor(name))) + 
  ...
  scale_x_continuous(breaks = min(dt_kenya_partners$year):max(dt_kenya_partners$year)) + 
...
#dt_kenya <- dt_kenya[dt_kenya$year == 2012,]
sample_size<-nrow(dt_kenya) #works with 100k
idx_sample<-sample(nrow(dt_kenya),sample_size)

docs<-Corpus(VectorSource(dt_kenya$description[idx_sample]))
...
# sort(colSums(help_words>5))
docs <- tm_map(docs, removeWords, c('kshs',
                                    'microfinance',
                                    'smep',
                                    'visionfund'))
...
dtmr_partner_id<-as.data.frame(cbind((dt_kenya$partner_id)[idx_sample],month(dt_kenya$posted_time)[idx_sample],year(dt_kenya$posted_time)[idx_sample],as.matrix(dtmr)))
...
save(dtmr_partner_id,file = "/Users/marcvalenti/TFM_data/dtmr_kenya.RData")
